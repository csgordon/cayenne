module example$tinterp_pe$abssyn =
...
substType :: (F :: Type -> #) -> (t :: Type) -> (t' :: Type) -> EqType t t' ->
     F t -> F t'
substType F (TInt) (TInt) _ = \f -> f
substType F (TInt) _ p = \_ -> absurd p
substType F (TBool) (TBool) _ = \f -> f
substType F (TBool) _ p = \_ -> absurd p
substType F (TArrow a r) (TArrow a' r') (pa & pr) =
    substType (\ (t::Type) -> F (TArrow t r')) a a' pa ·
    substType (\ (t::Type) -> F (TArrow a t)) r r' pr
substType F (TArrow a r) _ p = \(f :: F (TArrow a r)) -> absurd p
